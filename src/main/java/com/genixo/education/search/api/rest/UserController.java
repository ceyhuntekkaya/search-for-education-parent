...
public class UserController {
    ...
    public ResponseEntity<ApiResponse<List<UserSummaryDto>>> getUserBySchool(
            @Parameter(description = "campusId ID") @PathVariable Long campusId,
            HttpServletRequest request) {

        List<UserSummaryDto> user = userService.getUserByCampus(campusId);
        ApiResponse<List<UserSummaryDto>> response = ApiResponse.success(user, "User retrieved successfully");
        response.setPath(request.getRequestURI());
        response.setTimestamp(LocalDateTime.now());
