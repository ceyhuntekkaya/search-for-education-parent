...
public class AppointmentController {
    ...
    public ResponseEntity<ApiResponse<AppointmentDto>> createAppointment(
        ...
        log.info("Create appointment request for school: {} on date: {}", createDto.getSchoolId(), createDto.getAppointmentDate());
    ...
    @Operation(summary = "Reschedule appointment", description = "Reschedule an existing appointment to a new time")
    @ApiResponses(value = {
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "Appointment rescheduled successfully"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "404", description = "Appointment not found"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "403", description = "Access denied"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "400", description = "Cannot reschedule or no capacity available")
    })
    public ResponseEntity<ApiResponse<AppointmentDto>> rescheduleAppointment(
        ...
        log.info("Reschedule appointment request: {}", rescheduleDto.getAppointmentId());

        AppointmentDto appointmentDto = appointmentService.rescheduleAppointment(rescheduleDto, request);

        ApiResponse<AppointmentDto> response = ApiResponse.success(appointmentDto, "Appointment rescheduled successfully");
        response.setPath(request.getRequestURI());
        response.setTimestamp(LocalDateTime.now());
