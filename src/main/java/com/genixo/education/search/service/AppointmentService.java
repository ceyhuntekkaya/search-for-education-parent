...
public class AppointmentService {
    ...
    public AppointmentSlotDto createAppointmentSlot(AppointmentSlotCreateDto createDto, HttpServletRequest request) {
        ...
        slot.setSlotDate(createDto.getSlotDate());
        slot.setSchool(school);
        slot.setStaffUser(staffUser);
        slot.setDayOfWeek(createDto.getSlotDate().getDayOfWeek());
        slot.setStartTime(createDto.getSlotDate().toLocalTime());
        slot.setEndTime(createDto.getSlotDate().toLocalTime().plusMinutes(30));
        ...
        slot.setValidFrom(createDto.getSlotDate().toLocalDate());
        slot.setValidUntil(createDto.getSlotDate().toLocalDate());
    ...
    public AppointmentSlotBulkCreateResultDto createAppointmentSlotsBulk(AppointmentSlotBulkCreateDto bulkDto,
        ...
        List<AppointmentType> appointmentTypes = resolveBulkAppointmentTypes(bulkDto);
        List<Long> staffUserIds = resolveBulkStaffUserIds(bulkDto);
        ...
        Map<Long, User> staffUsersById = new HashMap<>();
        for (Long staffUserId : staffUserIds) {
            if (staffUserId == null) {
                continue;
            }
            User staffUser = userRepository.findByIdAndIsActiveTrue(staffUserId)
            ...
            staffUsersById.put(staffUserId, staffUser);
        }
        ...
        for (Long staffUserId : staffUserIds) {
            User staffUser = staffUserId != null ? staffUsersById.get(staffUserId) : null;
            ...
            for (AppointmentType appointmentType : appointmentTypes) {
                for (LocalDate date : bulkDto.getSelectedDates()) {
                    ...
                    for (TimeRange range : ranges) {
                        ...
                        boolean overlapping = appointmentSlotRepository.existsOverlappingSlotOnDate(
                                school.getId(),
                                staffUserId,
                                date,
                                dayOfWeek,
                        ...
                        String key = buildBulkSlotKey(staffUserId, appointmentType, date, range);
    ...
    public AppointmentDto createAppointment(AppointmentCreateDto createDto, HttpServletRequest request) {
        ...
        appointment.setAppointmentDate(slot.getSlotDate().toLocalDate());
        appointment.setStartTime(slot.getSlotDate().toLocalTime());
        appointment.setEndTime(slot.getSlotDate().toLocalTime().plusMinutes(30));
        ...
        appointment.setParentName(parentUser.getFirstName() + " " + parentUser.getLastName());
        appointment.setParentEmail(parentUser.getEmail());
        appointment.setParentPhone(parentUser.getPhone());
 ...
 */
    ...
    private List<Long> resolveBulkStaffUserIds(AppointmentSlotBulkCreateDto bulkDto) {
        if (bulkDto.getStaffUserIds() != null && !bulkDto.getStaffUserIds().isEmpty()) {
            return bulkDto.getStaffUserIds();
        }
        if (bulkDto.getStaffUserId() != null) {
            return List.of(bulkDto.getStaffUserId());
        }
        return List.of((Long) null);
    }

    private List<AppointmentType> resolveBulkAppointmentTypes(AppointmentSlotBulkCreateDto bulkDto) {
        if (bulkDto.getAppointmentTypes() != null && !bulkDto.getAppointmentTypes().isEmpty()) {
            return bulkDto.getAppointmentTypes();
        }
        if (bulkDto.getAppointmentType() != null) {
            return List.of(bulkDto.getAppointmentType());
        }
        throw new BusinessException("appointmentTypes or appointmentType is required");
    }

    private String buildBulkSlotKey(Long staffUserId, AppointmentType appointmentType,
                                    LocalDate date, TimeRange range) {
        return "staff=" + staffUserId
                + " type=" + appointmentType
                + " " + date + " " + range.startTime() + "-" + range.endTime();
    }
    ...
    public List<AppointmentSlotDto> searchSlotsWithDate(@Valid AppointmentSlotSearchDto searchDto) {
        List<AppointmentSlot> slots = appointmentSlotRepository.findByDateTimeRange(searchDto.getSchoolId(), searchDto.getStartDate().atStartOfDay(), searchDto.getEndDate().plusDays(1).atStartOfDay());
    ...
    @Transactional
    public AppointmentDto confirmAppointment(@Valid AppointmentConfirmDto confirmDto, HttpServletRequest request) {
        User user = jwtService.getUser(request);
        Appointment appointment = appointmentRepository.findByIdAndIsActiveTrue(confirmDto.getAppointmentId())
                .orElseThrow(() -> new ResourceNotFoundException("Appointment not found with ID: " + confirmDto.getAppointmentId()));
