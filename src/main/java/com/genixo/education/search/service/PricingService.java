...
public class PricingService {
    ...
    public CustomFeeDto createCustomFee(CustomFeeCreateDto createDto, HttpServletRequest request) {
        log.info("Creating custom fee: {} for pricing ID: {}", createDto.getFeeName(), createDto.getSchoolPricingId());

        User user = jwtService.getUser(request);

        SchoolPricing schoolPricing = schoolPricingRepository.findByIdAndIsActiveTrue(createDto.getSchoolPricingId())
                .orElseThrow(() -> new ResourceNotFoundException("SchoolPricing", createDto.getSchoolPricingId()));

        validateUserCanManageSchoolPricing(user, schoolPricing.getSchool().getId());

        // Validate fee data
        validateCustomFeeData(createDto);

        // Check if fee name already exists for this pricing
        if (customFeeRepository.existsByFeeNameIgnoreCaseAndSchoolPricingIdAndIsActiveTrue(
                createDto.getFeeName(), createDto.getSchoolPricingId())) {
        ...
        customFee.setSchoolPricing(schoolPricing);
        customFee.setCreatedByUser(user);
    ...
    @Cacheable(value = "custom_fees", key = "#pricingId")
    public List<CustomFeeDto> getCustomFeesByPricing(Long pricingId, HttpServletRequest request) {
        log.info("Fetching custom fees for pricing ID: {}", pricingId);
        ...
        List<CustomFee> customFees = customFeeRepository.findBySchoolPricingIdAndIsActiveTrueOrderByDisplayOrder(pricingId);
    ...
    public CustomFeeDto updateCustomFee(Long id, CustomFeeCreateDto updateDto, HttpServletRequest request) {
        ...
        validateUserCanManageSchoolPricing(user, customFee.getSchoolPricing().getSchool().getId());

        // Check fee name uniqueness if changed
        if (!customFee.getFeeName().equalsIgnoreCase(updateDto.getFeeName()) &&
                customFeeRepository.existsByFeeNameIgnoreCaseAndSchoolPricingIdAndIdNotAndIsActiveTrue(
                        updateDto.getFeeName(), customFee.getSchoolPricing().getId(), id)) {
