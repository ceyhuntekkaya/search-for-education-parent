...
public class ContentService {
    ...
    @Transactional
    public GalleryItemDto createGalleryItem(GalleryItemCreateDto createDto, HttpServletRequest request) {
        log.info("Adding item to gallery ID: {}", createDto.getGalleryId());

        User user = jwtService.getUser(request);
        Gallery gallery = galleryRepository.findByIdAndIsActiveTrue(createDto.getGalleryId())
        ...
        galleryRepository.incrementItemCount(gallery.getId());
        if (createDto.getFileSizeBytes() != null) {
            galleryRepository.addToTotalSize(gallery.getId(), createDto.getFileSizeBytes());
        }

        log.info("Gallery item created successfully with ID: {}", item.getId());
        return converterService.mapGalleryItemToDto(item);
    }
