...
public class UserConverterService {
    ...
    private final SubscriptionConverterService subscriptionConverterService;
    ...
    public UserRoleDto mapToDto(UserRole entity) {
        ...
        return UserRoleDto.builder()
                ...
                .subscription(subscriptionConverterService.mapToDto(entity.getSubscription()))
                .schools(new HashSet<>(institutionConverterService.mapSchoolsToDto(entity.getSchools())))
    ...
    public UserDto mapToDto(User entity) {
        ...
        BrandSummaryDto brandSummaryDto = null;
        CampusSummaryDto campusSummaryDto = null;
        SubscriptionSummaryDto subscriptionSummaryDto = null;


        Set<UserInstitutionAccess> institutionAccesses = entity.getInstitutionAccess();
        for (UserInstitutionAccess institutionAccess : institutionAccesses) {
            if (institutionAccess.getAccessType() == AccessType.BRAND) {
                Long brandId = institutionAccess.getEntityId();
                Brand brand = brandRepository.findById(brandId).orElse(null);
                brandSummaryDto = institutionConverterService.mapBrandToSummaryDto(brand);

            } else if (institutionAccess.getAccessType() == AccessType.CAMPUS) {
                Long campusId = institutionAccess.getEntityId();
                Campus campus = campusRepository.findById(campusId).orElse(null);
                campusSummaryDto = institutionConverterService.mapCampusToSummaryDto(campus);

                Subscription subscription = subscriptionRepository.findByCampusIdAndIsActiveTrue(campusId).orElse(null);
                subscriptionSummaryDto = subscriptionConverterService.mapToSummaryDto(subscription);
            }
        }
