...
public interface CustomFeeRepository extends JpaRepository<CustomFee, Long> {
    ...
    @Query("SELECT cf FROM CustomFee cf " +
            "WHERE cf.school.id = :schoolId AND cf.isActive = true " +
            "ORDER BY cf.displayOrder ASC, cf.feeName ASC")
    List<CustomFee> findBySchoolIdAndIsActiveTrueOrderByDisplayOrder(@Param("schoolId") Long schoolId);
    ...
    List<CustomFee> findMandatoryFeesByPricingId(@Param("schoolId") Long schoolId);
    ...
    @Query("SELECT COALESCE(SUM(cf.feeAmount), 0) FROM CustomFee cf " +
            "WHERE cf.school.id = :pricingId AND cf.isMandatory = true " +
            "AND cf.status = 'ACTIVE' AND cf.isActive = true")
    java.math.BigDecimal calculateTotalMandatoryFees(@Param("schoolId") Long schoolId);

    @Query("SELECT COALESCE(SUM(cf.feeAmount), 0) FROM CustomFee cf " +
            "WHERE cf.school.id = :pricingId " +
            "AND cf.status = 'ACTIVE' AND cf.isActive = true")
    java.math.BigDecimal calculateTotalCustomFees(@Param("schoolId") Long schoolId);
    ...
    @Query("SELECT cf FROM CustomFee cf " +
            ...
            "AND cf.school.id = :schoolId AND cf.isActive = true")
    List<CustomFee> findFeesRequiringDocumentation(@Param("schoolId") Long schoolId);

    @Query("SELECT cf FROM CustomFee cf " +
            "WHERE cf.installmentAllowed = true AND cf.status = 'ACTIVE' " +
            "AND cf.school.id = :schoolId AND cf.isActive = true")
    List<CustomFee> findInstallmentEligibleFees(@Param("schoolId") Long schoolId);
    ...
    @Query("SELECT cf FROM CustomFee cf " +
            ...
            "AND cf.school.id = :schoolId AND cf.isActive = true")
    List<CustomFee> findDiscountEligibleFees(@Param("schoolId") Long schoolId);

    @Query("SELECT cf FROM CustomFee cf " +
            "WHERE cf.scholarshipApplicable = true AND cf.status = 'ACTIVE' " +
            "AND cf.school.id = :schoolId AND cf.isActive = true")
    List<CustomFee> findScholarshipApplicableFees(@Param("schoolId") Long schoolId);
    ...
    @Query("SELECT cf FROM CustomFee cf " +
            ...
            "AND cf.school.id = :schoolId AND cf.status = 'ACTIVE' AND cf.isActive = true")
