...
import com.genixo.education.search.dto.survey.SurveyResponseDto;
 ...
 */
    ...
    @Query("SELECT sr FROM SurveyResponse sr WHERE sr.respondentUser.id =  :attendId")
    List<SurveyResponse> findByRespondentId(@Param("attendId") Long attendId);

    @Query("SELECT sr FROM SurveyResponse sr WHERE sr.school.id =  :schoolId")
    List<SurveyResponse> findBySchoolId(@Param("schoolId") Long schoolId);

    @Modifying
    @Query("DELETE FROM SurveyResponse sr WHERE sr.school.id = :schoolId")
